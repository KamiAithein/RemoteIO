        ...
        let stream = self.device.as_ref().expect("no audio device selected at connect!").build_input_stream(
            ...
            move |data: &[f32], _| {
                ...
                for data in data_chunks {
                    let to_send = remoteio_backend::BinMessages::BinData(remoteio_backend::AliasedData{alias: remoteio_backend::BinStreamAlias{alias: stream_name.clone()}, data: data.to_vec()});
...
impl SyncClient for Client {
    ...
    fn connect(&mut self, url: &str) -> Result<(), Box<dyn std::error::Error>> {
...

        {
            ...
            let bin_hello = bincode::serialize(&remoteio_backend::BinMessages::BinHello(remoteio_backend::BinStreamAlias { alias: alias.clone() })).expect("could not serialize hello message!");
        ...
        let bin_config = bincode::serialize(&remoteio_backend::BinMessages::BinConfig(remoteio_backend::AliasedData{alias: remoteio_backend::BinStreamAlias{alias: alias.clone()}, data: bin_config_struct})).expect("could not deserialize!");
