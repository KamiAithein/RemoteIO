...
pub mod audio;
use audio::devices::devicecoordinator::DeviceCoordinator;
...
fn main() -> Result <(), Box<dyn Error>> {
    ...
    let contains_string = |device: &cpal::Device, string: &str| device.name()
        .unwrap_or_default()
        .to_lowercase()
        .contains("vb-string"); 

    let virtual_input = coordinator.devices()?
        .filter(|device| contains_string(device, "vb-audio"))
        .filter(|device| contains_string(device, "input"))
        .last()
        .unwrap();

    let virtual_output = coordinator.devices()?
        .filter(|device| contains_string(device, "vb-audio"))
        .filter(|device| contains_string(device, "output"))
        .last()
        .unwrap();
